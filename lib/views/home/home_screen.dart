                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _SpeedoCards(),
                      const SizedBox(height: 16),
                      if (vm.isKycUnderReview)
                        _KycReviewBanner(onCheckStatus: _openKyc),
                      const SizedBox(height: 16),
                      _PromoBanner(
  ...
  void _onTap() {
    switch (label) {
      ...
      case 'My Bills':
        ...
        break;
      case 'New Plan':
        Navigator.push(screenContext, MaterialPageRoute(builder: (_) => const WifiPlansScreen()));
        break;
    }
  }
...
class _KycReviewBanner extends StatelessWidget {
  ...
  const _KycReviewBanner({required this.onCheckStatus});
...
class _SpeedoCards extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: _SpeedoCard(title: 'SPEEDO', titleSuffix: 'prime', isTv: false,
          subtitle: 'Watch your favourite\nmovies on Speedo Prime')),
      const SizedBox(width: 12),
      Expanded(child: _SpeedoCard(title: 'SPEEDO', titleSuffix: 'TV', isTv: true,
...
class _SpeedoCard extends StatelessWidget {
  ...
  final bool isTv;
  const _SpeedoCard({required this.title, required this.titleSuffix, required this.isTv, required this.subtitle});
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        RichText(text: TextSpan(children: [
          TextSpan(text: title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Colors.black, letterSpacing: 1)),
          if (isTv)
            const WidgetSpan(child: Padding(padding: EdgeInsets.only(left: 2), child: Icon(Icons.tv, size: 16, color: Colors.black)))
          else
            TextSpan(text: titleSuffix, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primary)),
        ])),
        const SizedBox(height: 8),
        Text(subtitle, style: const TextStyle(fontSize: 12, color: AppColors.textGrey)),
        const SizedBox(height: 12),
        const Row(children: [
          Text('Watch Now', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w700, fontSize: 13)),
          SizedBox(width: 4),
      ...
      child: Column(children: [
        ClipRRect(
          ...
          child: Container(
            ...
            decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF1A0A2E), Color(0xFF2D1060)])),
            child: Stack(alignment: Alignment.center, children: [
              Positioned(left: 16, child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                const SizedBox(height: 20),
                Text('SPEEDO PRIME PRESENTS', style: TextStyle(color: Colors.amber.shade300, fontSize: 10, fontWeight: FontWeight.w600, letterSpacing: 1.5)),
                const SizedBox(height: 6),
                const Text('MOVIE\nFESTIVAL', style: TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900, height: 1.1)),
                const SizedBox(height: 8),
                const Text('MARCH 30, THURSDAY', style: TextStyle(color: Colors.amber, fontSize: 11, fontWeight: FontWeight.w600)),
              ])),
              Positioned(right: 10, child: Icon(Icons.local_movies, size: 80, color: Colors.amber.withOpacity(0.6))),
            ]),
          ),
        ),
        Padding(
          padding: const EdgeInsets.symmetric(vertical: 10),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(2, (i) => Container(
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Features', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: AppColors.textDark)),
        ...
        Center(child: Column(children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFF5DEB3), borderRadius: BorderRadius.circular(20)),
            child: Stack(alignment: Alignment.center, children: [
